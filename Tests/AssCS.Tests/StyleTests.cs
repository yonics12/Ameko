...
public class StyleTests
{
    ...
    private const string V400PpStyle =
        "Style: Default,Noto Serif,69,&H00FFFFFF,&H00000000,&H0012291F,&HA012291F,-1,0,0,0,100,100,0,0,1,4.4,2.2,2,275,275,15,30,1,1";
    private const string SsaStyle =
        "Style: Planet_ES_main2,Slider,41,15987699,65535,65535,-2147483640,-1,0,1,1,2,2,30,30,30,0,0";
    ...
    {
        var s = Style.FromAss(1, BasicStyle, AssVersion.V400P);
    ...
    {
        var s = Style.FromAss(1, CcsStyle, AssVersion.V400P);
    ...
    {
        var s = Style.FromAss(1, SsaStyle, AssVersion.V400);
        await Assert.That(s).IsNotNull();
        await Assert.That(s.Name).IsEqualTo("Planet_ES_main2");
        await Assert.That(s.ShadowColor).IsEqualTo(Color.FromRgba(0x08, 0x00, 0x00, 0x80)); // -2147483640
    }

    [Test]
    public async Task FromAss_V400Pp()
    {
        var s = Style.FromAss(1, V400PpStyle, AssVersion.V400PP);
        await Assert.That(s).IsNotNull();
        await Assert.That(s.Name).IsEqualTo("Default");
        await Assert.That(s.Margins.Top).IsEqualTo(15);
        await Assert.That(s.Margins.Bottom).IsEqualTo(30);
    }

    [Test]
    public async Task FromAss_Malformed_ReturnsNull()
    {
        var style = Style.FromAss(
            1,
            "Mark's Pizzeria! Did you know kaakaadhiweijqojfofw",
            AssVersion.V400P
        );
    ...
    public async Task AsAss()
    {
        ...
        await Assert.That(s).IsNotNull();
        await Assert.That(s.AsAss(AssVersion.V400P)).IsEqualTo(BasicStyle);
    }

    [Test]
    public async Task Clone()
    {
        var s1 = Style.FromAss(1, BasicStyle, AssVersion.V400P)!;
        var s2 = s1.Clone();
