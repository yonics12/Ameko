...
public class EventTests
{
    ...
    private const string V400PpEvent =
        "Dialogue: 0,0:02:10.57,0:02:13.51,Default,Heiter,0,0,15,30,,It's the victorious return of the heroes' party.";
    private const string SsaEvent =
        "Dialogue: Marked=0,0:02:29.65,0:02:33.42,Planet_ES_main2,Fist-bangy-guy,0000,0000,0000,,I hear there have been warnings from the Space Defense Front!";
    ...
    {
        var e = Event.FromAss(1, BasicEvent, AssVersion.V400P);
    ...
    {
        var e = Event.FromAss(1, CcsEvent, AssVersion.V400P);
    ...
    [Test]
    public async Task FromAss_Ssa()
    {
        var e = Event.FromAss(1, SsaEvent, AssVersion.V400);
    ...
    [Test]
    public async Task FromAss_400Pp()
    {
        var e = Event.FromAss(1, V400PpEvent, AssVersion.V400PP);

        await Assert.That(e).IsNotNull();
        await Assert.That(e.Margins.Top).IsEqualTo(15);
        await Assert.That(e.Margins.Bottom).IsEqualTo(30);
    }
    ...
    public async Task FromAss_Malformed_ReturnsNull()
    {
        var e = Event.FromAss(
            1,
            "This portion of today's video is sponsored by Overstock.com",
            AssVersion.V400P
        );
    ...
    public async Task AsAss()
    {
        ...
        await Assert.That(e).IsNotNull();
        await Assert.That(e.AsAss(AssVersion.V400P)).IsEqualTo(BasicEvent);
    }

    [Test]
    public async Task AsAss_Ssa()
    {
        var e = Event.FromAss(1, SsaEvent, AssVersion.V400);

        await Assert.That(e).IsNotNull();
        await Assert.That(e.AsAss(AssVersion.V400)).Contains("Marked=0");
    }

    [Test]
    public async Task StripText()
    {
        var e = Event.FromAss(1, TagEvent, AssVersion.V400P)!;
        e.StripTags();
    ...
    {
        var e1 = Event.FromAss(1, BasicEvent, AssVersion.V400P)!;
        var e2 = e1.Clone();
    ...
    {
        var baseline = Event.FromAss(1, BasicEvent, AssVersion.V400P)!;
        var evt1 = Event.FromAss(1, BasicEvent, AssVersion.V400P)!;
        var evt2 = Event.FromAss(2, TagEvent, AssVersion.V400P)!;
    ...
    {
        var evt1 = Event.FromAss(1, BasicEvent, AssVersion.V400P)!;
        var evt2 = Event.FromAss(2, BasicEvent, AssVersion.V400P)!;
